...
- **D-P manual boundary:** `04_c5_controls/blind_quality_audit/packet_3/` is the current two-reviewer preparation packet. It preserves the original 30-person main validation sample, adds a separately analysed 13-person rare-domain enrichment sample and keeps ID 385 as a descriptive sentinel (44 validation participants; 440 stage-1 cells per reviewer); five training cases remain excluded from scoring. Stage 1 uses domain-specific human states and contains no C5/D-P/PHQ/model/sample-type fields. Stage 2 cannot be generated until the stage-1 consensus file is frozen, and it expands the actual retained C5 span rows rather than inferring rows from D-P counts. Owner keys and returned files remain under the ignored `blind_quality_audit/private/` directory. No agreement, consensus or D-P validity result is claimed before reviewer return. `packet_1/` and `packet_2/` are retained as historical preparation packets.
