...
**结论：可分享，但必须带限制条件（Share with caveats / CAUTION）。**

模块 14 在修复分块语义重复、错误分析输出契约、Fisher p 值精度、路径可移植性和公开隐私边界后，已满足原方案的核心要求。公开结果可作为 DAIC-WOZ 主分析的探索性补充，但不能称为严格外部验证，也不能把小样本点估计解释为稳定性能。
...
| 原方案要求 | 验收结果 | 证据或说明 |
|---|---|---|
| DAIC-WOZ 为主分析，E-DAIC 仅作探索性补充 | 通过 | summary 明确禁止“严格外部验证”表述 |
...
| 错位、一致及四象限错误明细 | 通过（修复后） | 输出 42 行：3 模型 x 2 证据定义 x 7 分组 |
| Fisher 精确检验仅作探索性 | 通过（修复后） | 保留完整 p 值；note 明确 6 次比较未经校正 |
| DAIC-WOZ 与 E-DAIC 对照表 | 通过 | 样本、错位比例、三模型 AUC 和两类错分率均已并排 |
...
去重使 participant 680 和 684 的 evidence density 各下降 1；固定切点和四象限人数不变。base AUC 从 0.6979 调整为 0.6875，presence AUC 从 0.5938 调整为 0.5833，研究方向未改变。
...
| pres | 0.5833 | 0.3125-0.8438 | 0.3435 | 0.5909 | 0.8333 | 0.5000 | 5/8/1/8 |

三种 AUC 的区间均跨过 0.5，因此只能报告点估计和方向，不能声称在 E-DAIC 小样本中已证明稳定判别性能。

### 错位与错误集中

| 模型/定义 | 错位错误 | 一致错误 | 风险差 | Fisher p | 审计用 BH q |
|---|---:|---:|---:|---:|---:|
| base / coverage | 9/9 | 0/13 | 1.0000 | 2.010e-06 | 1.206e-05 |
| base / density | 8/9 | 1/13 | 0.8120 | 2.372e-04 | 3.558e-04 |
| count / coverage | 6/9 | 3/13 | 0.4359 | 0.0789 | 0.0789 |
| count / density | 6/9 | 3/13 | 0.4359 | 0.0789 | 0.0789 |
| pres / coverage | 8/9 | 1/13 | 0.8120 | 2.372e-04 | 3.558e-04 |
| pres / density | 8/9 | 1/13 | 0.8120 | 2.372e-04 | 3.558e-04 |

BH q 仅用于本审计评估多重比较风险，主输出仍保留方案指定的探索性 Fisher p，并明确其未经校正。count 模型的差异不能按常用 0.05 阈值解释为显著。
...
| Look-elsewhere effect | CAUTION | 3 模型 x 2 定义共 6 次 Fisher 检验；主文不得选择性强调 p 值 |
| Garden of forking paths | NOTE | 模型与固定阈值来自既定方案；API 分块是运行期修复，已完整披露并固定参数 |
| Correlation is not causation | 无异常 | summary 使用“观察到”“方向上支持”“错误集中”，未作因果断言 |
...
- 公开指标由预测表独立复算，所有 AUC、PR-AUC、分类指标、混淆矩阵和 Fisher p 与输出精确一致。
...
4. AUC 区间很宽且跨过 0.5；性能结论只能作探索性描述。
5. 错位与错误集中是关联性观察，不应写成因果机制。
6. PHQ-8 是自评症状量表，不是临床诊断。
