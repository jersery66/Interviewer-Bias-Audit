"""Apply all 10 fixes to run_symptom_level_analysis.py in one pass."""
import re
...
assert old in c, f"FIX 1 failed: '{old}' not found"
c = c.replace(old, new)
print("FIX 1 ✓ BASE → relative path")

# ═══════════════════════════════════════════════════════════════════
# FIX 2: Add seed param to prediction_swap_permutation_test()
# ═══════════════════════════════════════════════════════════════════
old = 'def prediction_swap_permutation_test(y_true, prob_a, prob_b, n_perm=PERMUTATION_N):'
new = 'def prediction_swap_permutation_test(y_true, prob_a, prob_b, n_perm=PERMUTATION_N, seed=None):'
assert old in c, "FIX 2a failed"
c = c.replace(old, new)
print("FIX 2a ✓ seed param added")

old2 = '    rng = np.random.RandomState(RANDOM_SEED)'
new2 = '    rng = np.random.RandomState(seed if seed is not None else RANDOM_SEED)'
assert old2 in c, "FIX 2b failed"
c = c.replace(old2, new2)
print("FIX 2b ✓ RNG uses seed")

# ═══════════════════════════════════════════════════════════════════
# FIX 3: M7 – permutation for p, bootstrap for CI only
# ═══════════════════════════════════════════════════════════════════
old = '    for name_a, name_b, prob_a, prob_b in comparisons:'
new = '    for idx, (name_a, name_b, prob_a, prob_b) in enumerate(comparisons):'
assert old in c, "FIX 3a failed"
c = c.replace(old, new)

old = '        delta, ci_l, ci_u, p_val = paired_bootstrap_delta_auc(y, prob_a.values, prob_b.values)'
rep = '''        # Bootstrap for CI only (not p-value)
...
if start >= 0:
    rest = c[start:]
    lines = rest.split('\n')
    # Find end of function: next def/function at same indent level
    ...
    for i in range(1, len(lines)):
        ...
        if indent <= base_indent and lines[i].strip().startswith('def '):
...
assert old in c, "FIX 7 failed: M4/M5 call not found"
ins = (
    '    coverage_tbl, coverage_trend, coverage_cv = module4_coverage_gradient(dpres, labels)\n'
    '    density_tbl, density_trend, density_cv = module5_evidence_density_gradient(dcount, labels)\n'
...
c = c[:main_pos] + fine_funcs + c[main_pos:]
print("FIX 10a ✓ fine-grained functions added")

# Add calls in main()
old = '    density_tbl, density_trend, density_cv = module5_evidence_density_gradient(dcount, labels)\n    # Combined trend FDR'
new = '    density_tbl, density_trend, density_cv = module5_evidence_density_gradient(dcount, labels)\n    # Fine-grained supplementary gradients\n    module4_coverage_gradient_fine(dpres, labels)\n    module5_evidence_density_gradient_fine(dcount, labels)\n    # Combined trend FDR'
c = c.replace(old, new)
