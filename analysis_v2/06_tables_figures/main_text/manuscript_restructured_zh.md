...
各文本来源采用折内 TF-IDF 和逻辑回归建模，文本表示、数值标准化和参数拟合均只在训练折完成。主分析使用固定的受试者级五折划分，每名被试获得一次交叉拟合预测；重复交叉验证仅用于补充稳定性分析，不把重复折视为新增独立样本。结构模型只使用长度、互动或协议变量；联合模型先保留被试语言、症状域计数和协议模板，再分别加入访谈者语言或症状证据表示，以估计条件增量。
...
在基础联合模型中加入访谈者语言，AUC 仅增加 0.0028（95% CI −0.0040–0.0101）；加入症状证据表示的增量为 0.0035（−0.0038–0.0114）。两者都未显示稳定的条件增量。访谈者语言本身的预测表现较高，不能因此推断访谈者语言在被试语言、症状域和协议模板之外仍提供同等规模的独立信息。
...
本研究的核心结果是，完整转录中的预测信息并非全部来自被试语言。访谈者语言、协议结构和综合过程变量也含有标签相关信息，但在保留被试语言、症状域和协议模板后，访谈者语言的条件增量有限。由此，单来源 AUC 应被理解为预测充分性，而不是独立贡献或机制证据。
...
本研究仍受样本量较小、同一数据集内部交叉验证、观察性设计、有限的文本预算控制、来源对齐复核未能证明结果盲法且没有双人一致性、缺乏独立外部验证等限制。公开仓库可以验证代码、测试、合成样例、冻结派生结果和哈希，但在受限原始数据与私有审核源不可公开的前提下，不能宣称从原始数据完整复现全部分析。因此，完整转录模型的性能不应未经来源分解便归因于被试自身症状语言。半结构化访谈模型的解释应同时考虑说话者来源、协议结构、文本预算和概率校准，而不能仅依据单一 AUC 作出偏差、泄漏或临床有效性的判断。
...
症状证据候选共 1143 条，最终保留 1137 条，其中 1102 条无需来源修订，35 条进行了逐字来源对齐，5 条重复证据和 1 条无效证据被排除。最终保留片段的 1137/1137 追溯率是审核后的可追溯性，不是抽取准确率；35 条修订涉及 24 名被试，改变了 24 名被试的表示，空格分词总量增加 103。审核表包含 PHQ-8 分数或标签列，不能证明结果盲法；没有记录复核者身份或第二名独立复核者，无法报告双人一致性。
