...
单来源模型回答预测充分性。联合模型 M3 包含被试语言、参与者语言衍生的十个症状域计数（D-P）和协议模板出现；M4 在 M3 上加入 C5 症状证据文本，M5 在 M3 上加入访谈者文本。当前没有通过来源和质量核查的全访谈 D-All 输入，因此 M3/M5 不应被描述为控制了完整访谈中的症状域。联合增量只解释为当前特征与模型条件下的条件关联。
...
更重要的是，在保留被试语言、D-P 症状域计数和模板出现的 M3 中，新增访谈者文本仅产生 0.0028 的 AUC 增量（−0.0040 至 0.0101；p=0.3785，q=0.4731）。这一区分表明：访谈者语言单独可预测，但在当前联合表征中未检测到可区分的额外贡献；它不等于在 D-All 条件下的结果。不能从访谈者单来源 AUC 推导“访谈者比被试更重要”。
...
8. 单来源 TF-IDF 和线性联合模型只代表当前操作化；D-P 是参与者语言衍生控制，D-All 尚未具备可审计输入；相关来源与正则化可互相替代，条件增量接近零不证明绝对无信息。
