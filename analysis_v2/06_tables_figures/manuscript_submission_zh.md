...
本文的分析范围有三项明确限制。第一，D 是由被试语言衍生的十个症状域计数（D-P），不是从完整访谈重新提取的 D-All。第二，所有新增分析均为审计后提出的敏感性分析，不是预注册。第三，D-P 的人工盲法内容效度核查目前仅完成材料准备：保留原三十人主验证样本，另设十三人稀有域富集样本和 ID 385 哨兵个案，并提供五例不计分培训样本。两名独立复核者尚未回填，因此本文不报告人工一致率、Cohen’s κ 或人工共识 F1。
...
D-P 盲法内容效度包保留按标签×被试词数三分位抽取的原 30 人主验证样本（每格 5 人，种子 20260714），另按最小覆盖原则加入 13 人稀有域富集样本，并将 ID 385 设为不进入总体统计的哨兵个案；5 人培训样本不计入验证。第一阶段复核者只获得统一盲编号、完整被试语言、1-based 行号、域名称和冻结 codebook，不获得 C5 证据、D-P presence/count、PHQ-8、模型结果或样本类型。通用症状域采用 `current_present`、`explicitly_absent`、`past_only`、`conflicting` 和 `indeterminate`，心理健康史与保护/否认域使用各自目标状态；同时记录 0/1/2/3+ 计数、逐字证据、时间、否定和主体。两名复核者先独立编码并计算一致性，人工共识冻结后才可打开第二阶段；第二阶段按实际 C5 span 行逐条审核来源支持、说话者、域、否定、时间、主体、上下文、重复和遗漏，不根据 D-P count 或 `3+` 推定证据条数。主要二分类将 `current_present` 映射为阳性，其余非冲突状态映射为非当前阳性，`conflicting` 从主要分析排除并分别按 0 和 1 编码作敏感性分析；报告敏感度、特异度、阳性类 F1、Macro-F1、Cohen’s κ 和参与者聚类 bootstrap 95% CI。主要效度只估计原 30 人，富集样本与 ID 385 分别报告；count 与具体证据错误仅作次要结果。
...
6. D-P 盲法核查的 packet_3 已完成第一阶段材料准备，但尚未有两名复核者的独立编码；第二阶段仍被共识冻结闸门锁定。在此之前不报告 Cohen’s κ、共识 F1 或人工准确性。若只有一名复核者，结果只能称为盲法抽样内容效度检查。
