...
D-P 盲法内容效度包保留按标签×被试词数三分位抽取的原 30 人主验证样本（每格 5 人，种子 20260714），另按最小覆盖原则加入 13 人稀有域富集样本，并设置 1 名预先指定且不进入总体统计的哨兵个案；5 人培训样本不计入验证。第一阶段复核者只获得统一盲编号、完整被试语言、1-based 行号、域名称和冻结 codebook，不获得 C5 证据、D-P presence/count、PHQ-8、模型结果或样本类型。通用症状域采用 `current_present`、`explicitly_absent`、`past_only`、`conflicting` 和 `indeterminate`，心理健康史与保护/否认域使用各自目标状态；同时记录 0/1/2/3+ 计数、逐字证据、时间、否定和主体。两名复核者独立编码后形成冻结共识。域级二分类中，通用症状域的 `current_present`、心理健康史域的 `history_present` 以及保护/否认域的 `protective_denial_evidence_present` 映射为阳性，其余非冲突状态映射为非阳性；`conflicting` 从主要分析排除，并分别按 0 和 1 编码作敏感性分析。主要效度只估计原 30 人，并报告敏感度、特异度、阳性类 F1、Macro-F1、Cohen’s κ 和参与者聚类 bootstrap 95% CI；富集样本与哨兵个案分别报告，count 仅作次要结果。第二阶段逐条证据审核不作为当前主论文的完成门槛，其未经最终裁决的证据质量和重要遗漏指标不进入正式结论。
