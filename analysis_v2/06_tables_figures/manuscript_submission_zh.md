...
被试语言、访谈者语言、完整转录和被试症状证据的 AUC 分别为 0.717、0.808、0.760 和 0.735。访谈者语言相对于已包含被试语言、协议结构和 D-P 症状域的强基线，其条件 ΔAUC 为 0.0028（95% CI −0.0040 至 0.0101，BH q=0.473）。在独立盲法主验证样本中，D-P 域级 presence 对人工共识的准确率为 0.799、敏感度为 0.792、特异度为 0.805，Cohen’s κ 为 0.594，因而被限定为不完美的症状代理。模块 15 的交叉拟合结果显示，P、Q、R、D 均可在不同表示下部分重建访谈者侧预测分数，但 R 的统计恢复能力通常最大；Shapley 输出被作为重叠信息的描述性分解，不被解释为信号百分比。完整 P+Q+R+D 后的残余标签增量随 TF-IDF、MPNet 和 BGE 表示改变，未形成稳定方向；该正式结果的运行状态标记为 `training_match_quality_limited`，因此其配对部分不作为主要识别证据。
...
本文的分析范围有三项明确限制。第一，D 是由被试语言衍生的十个症状域计数（D-P），不是从完整访谈重新提取的 D-All。第二，所有新增分析均为审计后提出的敏感性分析，不是预注册。第三，D-P 的第一阶段人工盲法域级效度核查已经完成，但只验证 presence/count 代理；第二阶段逐条证据质量和重要遗漏裁决未被纳入正式结果。因此，本文不把 D-P 写成临床金标准或完整症状证据集。
...
逐级条件模型按“被试语言 → 被试语言+协议/互动结构 → 加 D-P → 加访谈者语言”的顺序报告。条件增量以配对 OOF 预测的 ΔAUC 为主，同时保留 PR-AUC、Brier 和 log loss 的差值。概率质量使用外层训练折阳性率构建的正式无技能基线，并以 Brier skill score (BSS) 报告：
...
D-P 盲法内容效度包保留按标签×被试词数三分位抽取的原 30 人主验证样本（每格 5 人，种子 20260714），另按最小覆盖原则加入 13 人稀有域富集样本，并将 ID 385 设为不进入总体统计的哨兵个案；5 人培训样本不计入验证。第一阶段复核者只获得统一盲编号、完整被试语言、1-based 行号、域名称和冻结 codebook，不获得 C5 证据、D-P presence/count、PHQ-8、模型结果或样本类型。通用症状域采用 `current_present`、`explicitly_absent`、`past_only`、`conflicting` 和 `indeterminate`，心理健康史与保护/否认域使用各自目标状态；同时记录 0/1/2/3+ 计数、逐字证据、时间、否定和主体。两名复核者独立编码后形成冻结共识。域级二分类中，通用症状域的 `current_present`、心理健康史域的 `history_present` 以及保护/否认域的 `protective_denial_evidence_present` 映射为阳性，其余非冲突状态映射为非阳性；`conflicting` 从主要分析排除，并分别按 0 和 1 编码作敏感性分析。主要效度只估计原 30 人，并报告敏感度、特异度、阳性类 F1、Macro-F1、Cohen’s κ 和参与者聚类 bootstrap 95% CI；富集样本与哨兵个案分别报告，count 仅作次要结果。第二阶段逐条证据审核不作为当前主论文的完成门槛，其未经最终裁决的证据质量和重要遗漏指标不进入正式结论。
...
**表 2. 配对破坏与 Fake-D 的 10-repeat 主要对比**
...
### 3.5 D-P 域级人工效度：可作为代理，但并不完整

在主 30 人的 300 个“参与者×症状域”单元中，两名人工复核者的状态完全一致率为 0.870，Cohen’s κ 为 0.810（参与者聚类 bootstrap 95% CI 0.739–0.872）。排除 1 个 `conflicting` 单元后，自动 D-P presence 对人工共识的准确率为 0.799（95% CI 0.760–0.839），敏感度为 0.792（0.709–0.868），特异度为 0.805（0.750–0.860），阳性类 F1 为 0.774（0.711–0.829），Macro-F1 为 0.797（0.754–0.836），Cohen’s κ 为 0.594（0.509–0.673）。`conflicting` 分别按阴性和阳性处理时，准确率分别为 0.797 和 0.800，结论未改变。

Count 的 0/1/2/3+ 四级完全一致率为 0.643，线性加权 κ 为 0.599，二次加权 κ 为 0.701，平均绝对差为 0.487，Spearman ρ 为 0.758。上述结果支持 D-P 在域级 presence 上具有中等人工效度，但计数一致性较弱；它不证明具体证据片段全部正确，也不证明原文证据已被完整覆盖。13 人富集样本和哨兵个案不并入主要总体估计。

汇总指标见补充表 S1；逐域稀有单元仅作描述性结果，不据此作强推断。
...
C5 是临床导向的衍生表示，不是独立自然语言来源。最终保留片段的来源对齐修订只改变了逐字 quote，域、极性和保留状态固定；AUC 变化 −0.0042 的区间跨零。因此，该修订主要改善可追溯性，不能据此声称人工审核前后完整性能未变。第一阶段双人盲法验证表明 D-P 的域级 presence 具有中等人工效度，但并非无误代理；第二阶段证据级审核尚未形成最终裁决，因而本文不报告最终重要遗漏率，也不把域级效度外推为逐条证据完整性。
...
6. D-P 第一阶段域级盲法验证只覆盖 30 人主样本，且将 `indeterminate` 作为非当前阳性纳入主要二分类；虽然敏感性分析稳定，该操作化仍可能影响特异度。第二阶段证据级评分虽已回收，但“重要遗漏”标准存在系统性评价者差异且未完成最终裁决，因此本文不报告最终证据级遗漏率。
...
代码、测试、冻结输入的哈希、交叉拟合派生结果和正式敏感性输出保存在 `analysis_v2/`。配对/Fake-D 正式结果入口为 `analysis_v2/10_source_importance/08_identification_sensitivity/run_formal_10x/`；模块 15 结果入口为 `analysis_v2/15_interviewer_signal_explanation/final/`；D-P 盲法包入口为 `analysis_v2/04_c5_controls/blind_quality_audit/packet_3/`。公开 CI 只验证 code/test reproducibility、合成 fixture、schema/路径检查、图表可执行性以及已提交派生结果的哈希一致性。没有 DAIC-WOZ 受限原始数据和私有 C5 审计源时，不声称干净克隆可以从原始数据完整复现全部结果。
