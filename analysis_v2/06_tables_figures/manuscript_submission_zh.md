...
本文的分析范围有三项明确限制。第一，D 是由被试语言衍生的十个症状域计数（D-P），不是从完整访谈重新提取的 D-All。第二，所有新增分析均为审计后提出的敏感性分析，不是预注册。第三，D-P 的人工盲法内容效度核查目前已锁定五例培训样本、三十例正式样本、逐行文本、十域计数和 NS 规则，但尚未得到两名独立复核者的回填，因此本文不报告人工一致率、Cohen’s κ 或人工共识 F1。
...
D-P 盲法内容效度包按标签×被试词数三分位分层抽取 30 人（每格 5 人，种子 20260714），另提供不计入统计的 5 人培训样本。复核者只获得盲编号、被试语言、1-based 行号和冻结 codebook；每人逐域记录 0/1/NS、0/1/2/3+ 计数、极性、逐字证据和不确定性理由。盲编号键及 D-P 参考表保存在忽略的 private 目录；在两份正式回填文件锁定并先计算独立一致性前，不解盲、不生成共识，也不计算 D-P precision、recall 或 F1。
...
6. D-P 盲法核查的 packet_2 已准备完成，但尚未有两名复核者的独立编码；在此之前不报告 Cohen’s κ、共识 F1 或人工准确性。若只有一名复核者，结果只能称为盲法抽样内容效度检查。
...
代码、测试、冻结输入的哈希、交叉拟合派生结果和正式敏感性输出保存在 `analysis_v2/`。配对/Fake-D 正式结果入口为 `analysis_v2/10_source_importance/08_identification_sensitivity/run_formal_10x/`；模块 15 结果入口为 `analysis_v2/15_interviewer_signal_explanation/final/`；D-P 盲法包入口为 `analysis_v2/04_c5_controls/blind_quality_audit/packet_2/`。公开 CI 只验证 code/test reproducibility、合成 fixture、schema/路径检查、图表可执行性以及已提交派生结果的哈希一致性。没有 DAIC-WOZ 受限原始数据和私有 C5 审计源时，不声称干净克隆可以从原始数据完整复现全部结果。
