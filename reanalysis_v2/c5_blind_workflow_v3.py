...
def _quote_line_numbers(blind_id: str, quote: str, lines: pd.DataFrame) -> str:
    subset = lines.loc[lines["blind_id"].astype(str).eq(str(blind_id))].copy()
    subset["_line_number_numeric"] = pd.to_numeric(subset["line_number"], errors="raise")
    subset = subset.sort_values("_line_number_numeric", kind="stable")
    ...
    if matches or not normalized_quote:
        return ";".join(str(value) for value in matches)

    # C5 exact quotes may span consecutive participant-text lines.  Match the
    # quote against the normalized joined transcript and return every source
    # line intersected by the match instead of leaving the line number blank.
    joined_parts: list[str] = []
    intervals: list[tuple[int, int, int]] = []
    cursor = 0
    for row in subset.itertuples(index=False):
        text = _normalize_space(row.participant_text_line)
        if not text:
            continue
        if joined_parts:
            cursor += 1
        start = cursor
        cursor += len(text)
        intervals.append((start, cursor, int(row.line_number)))
        joined_parts.append(text)
    joined = " ".join(joined_parts)
    start = joined.find(normalized_quote)
    if start < 0:
        return ""
    end = start + len(normalized_quote)
    return ";".join(
        str(line_number)
        for line_start, line_end, line_number in intervals
