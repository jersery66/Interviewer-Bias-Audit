...
def test_stage2_maps_multiline_exact_quote_to_all_source_lines():
    consensus = pd.DataFrame(
        {
            "blind_id": ["V001"],
            "domain": ["depressed_mood"],
            "consensus_evidence_quote": ["I feel down sometimes"],
            "consensus_line_number": ["1"],
        }
    )
    key = pd.DataFrame(
        {"blind_id": ["V001"], "participant_id": [101], "sample_type": ["main"]}
    )
    spans = pd.DataFrame(
        {
            "participant_id": [101],
            ...
            "domain": ["depressed_mood"],
            "polarity": ["present"],
            "exact_quote": ["I feel down sometimes my life is strange"],
        }
    )
    lines = pd.DataFrame(
        {
            "blind_id": ["V001", "V001"],
            "line_number": [1, 2],
            "participant_text_line": ["I feel down sometimes", "my life is strange"],
        }
    )

    evidence, _ = build_stage2_evidence_forms(
        consensus,
        key,
        spans,
        lines,
    ...
    assert evidence.loc[0, "dp_line_number"] == "1;2"
