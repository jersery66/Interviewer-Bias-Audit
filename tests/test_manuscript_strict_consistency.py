...
def test_claim_matrix_and_submission_readme_lock_interpretation_boundaries() -> None:
    ...
    assert "human status agreement=0.870" in matrix
    assert "D-P presence versus consensus" in matrix
    assert "accuracy=0.799" in matrix
    assert "no agreement or consensus metrics are available" not in matrix
